...
local PuzzleMessages = {
    ...
    completion = {
        ...
        "Construction Hell yields to understanding."
    }
}
...
local function ResetPuzzle()
    ...
    Chat.BroadcastMessage("The sequence resets... begin again.")
end
...
local function SpawnConstructionHellContent(dimension)
    ...
    for i, prop_data in ipairs(PuzzleProps) do
        ...
        puzzle_prop:Subscribe("Interact", function(self, character)
            ...
            if self.prop_type == expected_type then
                ...
                Chat.BroadcastMessage("Step " .. current_step .. " of " .. ConstructionHellPuzzle.sequence_length .. ": " .. self.prop_name)
                ...
                for p, _ in ipairs(ConstructionHellPuzzle.puzzle_props) do
                    if p:IsValid() then
    ...
    local instruction_text = TextRender(
        Vector(0, 0, 450),
        Rotator(),
        "Touch the objects in the correct order to escape",
        Vector(2, 2, 2),
        Color(1, 1, 1),
        FontType.OpenSans,
        TextRenderAlignCamera.FaceCamera
